...
def split_dataset(df, number, target, feature_encoding, random_state=42):
    ...
    try:
        ...
        if 'Word Count' in feature_encoding:
            feature_pref.append('word_count_')
        if 'TF-IDF' in feature_encoding:
            feature_pref.append('tf_idf_word_count_')

        X_train_sentiment = X_train.loc[:, X_train.columns.str.startswith(tuple(feature_pref))]
        X_val_sentiment = X_val.loc[:, X_val.columns.str.startswith(tuple(feature_pref))]
        ...
        st.markdown('The training dataset contains {0:.2f} observations ({1:.2f}%) and the test dataset contains {2:.2f} observations ({3:.2f}%).'.format\
...
def train_sgd_classifer(X_train, y_train, model_name, params, random_state=42):
    ...
    # Add code here
    try:
        sgd_model = SGDClassifier(random_state=random_state, loss= 'log', penalty=params['penalty'], alpha=params['alpha'], \
...
def train_sgdcv_classifer(X_train, y_train, model_name, params, cv_params, random_state=42):
    ...
    sgd_cv_model = None
    try:
        #gd_cv = RepeatedKFold(n_splits=cv_params['n_splits'], n_repeats=cv_params['n_repeats'], random_state=random_state)
        params['loss'] = ['log']
...
def inspect_coefficients(trained_models):
    ...
    for name, model in trained_models.items():
        if model is not None:
            # if name == 'Stochastic Gradient Descent with Cross Validation':
            #     model = model.best_estimator_
            coefficients = model.coef_
            out_dict[name] = coefficients
            st.write(f"##### Model: {name}")
            st.write(f"Coefficients: {coefficients}")
            flattened_coefficients = coefficients.flatten()
            st.write(f"Total number of coefficients: {flattened_coefficients.shape[0]}")
            st.write(f"Number of positive coefficients: {(flattened_coefficients > 0).sum()}")
            st.write(f"Number of negative coefficients: {(flattened_coefficients < 0).sum()}")
...
if df is not None:
    ...
    if (classification_methods_options[0] in classification_model_select or classification_methods_options[0] in trained_models):
        ...
        with (lg_col1):
            ...
            lg_max_iter = st.number_input(
                label='Enter the number of max iterations on training data',
                min_value=1000,
                max_value=5000,
                value=1000,
                step=100,
    ...
    # Stochastic Gradient Descent with Logistic Regression with Cross Validation
    if (classification_methods_options[2] in classification_model_select or classification_methods_options[2] in trained_models):
        ...
        with (sdg_col):
            ...
            # )
            sgdcv_tol = float('0.01')
            # st.write('You select the following tolerance value: {}'.format(sgdcv_tol))
        ...
        if st.button('Train Stochastic Gradient Descent Model with Cross Validation'):
            trained_model = train_sgdcv_classifer(
                X_train, y_train, classification_methods_options[2], sgdcv_params, sgdcv_cv_params)
            st.session_state[classification_methods_options[2]] = trained_model
    ...
    # Inspect classification coefficients
