...
def summarize_review_data(df, reviews_col, top_n=3):
    ...
    st.write('- Average word count: {}'.format(np.round(out_dict['average_word_count'],2)))

    st.write('- Top 3 reviews with the highest number of words')
    st.dataframe(out_dict['top_n_reviews_most_words'])

    st.write('- Top 3 reviews with least number of words:')
